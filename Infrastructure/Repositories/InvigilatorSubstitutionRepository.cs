    ...
    public class InvigilatorSubstitutionRepository : IInvigilatorSubstitutionRepository
    {
        ...
        public async Task<List<int>> GetBusyLecturerIdsAsync(IEnumerable<int> userIds, int slotId, DateOnly examDate, CancellationToken cancellationToken = default)
        {
            ...
            var busyPersonKeys = await _db.LecturerBusySlots.AsNoTracking()
                .Where(x => personKeySet.Contains(x.User.InformationId > 0 ? x.User.InformationId : x.UserId) && x.ApprovalStatus == BusyApprovalStatuses.Approved && x.SlotId == slotId && x.BusyDate == examDate)
        ...
        public async Task<HashSet<int>> GetPeriodAvailableLecturerIdsAsync(int periodId, int facultyId, IEnumerable<int> userIds, CancellationToken cancellationToken = default)
        {
            var ids = userIds.Distinct().ToList();
            var candidatePersonKeys = await GetPersonKeysByUserIdAsync(ids, cancellationToken);
            var rows = await _db.LecturerPeriodAvailabilities.AsNoTracking()
        ...
        public Task<bool> HasPeriodAvailabilityListAsync(int periodId, int facultyId, CancellationToken cancellationToken = default)
        {
            return _db.LecturerPeriodAvailabilities.AsNoTracking()
                .AnyAsync(x => x.PeriodId == periodId && x.User.FacultyId == facultyId, cancellationToken);
        }

        public async Task<HashSet<int>> GetApprovedBusyPeriodLecturerIdsAsync(int periodId, IEnumerable<int> userIds, CancellationToken cancellationToken = default)
        {
            var ids = userIds.Distinct().ToList();
            var candidatePersonKeys = await GetPersonKeysByUserIdAsync(ids, cancellationToken);
            var personKeySet = candidatePersonKeys.Values.ToHashSet();
            var busyPersonKeys = await _db.LecturerBusyPeriods.AsNoTracking()
        ...
        public async Task<InvigilatorSubstitutionDetailDto?> GetDetailAsync(int substitutionId, int facultyId, CancellationToken cancellationToken = default)
        {
            return await _db.InvigilatorSubstitutions.AsNoTracking()
                ...
                .Select(x => new InvigilatorSubstitutionDetailDto
                {
                    ...
                    RoomDisplay = x.ExamInvigilator.ExamSchedule.Room.BuildingId == "KHAC" ? x.ExamInvigilator.ExamSchedule.Room.RoomName : x.ExamInvigilator.ExamSchedule.Room.BuildingId + "." + x.ExamInvigilator.ExamSchedule.Room.RoomName,
        ...
        private IQueryable<InvigilatorSubstitutionScheduleDto> BuildAssignmentQuery()
        {
            return _db.ExamInvigilators.AsNoTracking().Select(x => new InvigilatorSubstitutionScheduleDto
            {
                ...
                RoomDisplay = x.ExamSchedule.Room.BuildingId == "KHAC" ? x.ExamSchedule.Room.RoomName : x.ExamSchedule.Room.BuildingId + "." + x.ExamSchedule.Room.RoomName,

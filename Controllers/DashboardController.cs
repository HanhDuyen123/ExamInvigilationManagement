...
namespace ExamInvigilationManagement.Controllers
{
    ...
    public class DashboardController : Controller
    {
        ...
        {
            var today = DateTime.Today;
            var overdue = await _db.ExamInvigilators.CountAsync(x => x.AssigneeId == userId && x.ExamSchedule.Status == ExamScheduleStatuses.Approved && !x.InvigilatorResponses.Any(r => r.UserId == userId) && x.ExamSchedule.ExamDate.Date <= today.AddDays(3), cancellationToken);
            model.WorkItems.Add(new() { Title = "Chưa phản hồi", Description = "Lịch coi thi cần xác nhận hoặc từ chối.", Count = await _db.ExamInvigilators.CountAsync(x => x.AssigneeId == userId && x.ExamSchedule.Status == ExamScheduleStatuses.Approved && !x.InvigilatorResponses.Any(r => r.UserId == userId), cancellationToken), Url = Url.Action("Index", "InvigilatorResponse", new { area = "Lecturer", status = "Chưa phản hồi" }) ?? "#", Icon = "bi-check2-square", Tone = overdue > 0 ? "danger" : "warning", BadgeText = overdue > 0 ? $"{overdue} lịch sát ngày thi" : null });
